...
| **Scout**                | The Tech Intelligence Agent (`backend/factory/scout_agent.py`). Scans the tool landscape and writes Evolution Proposals to `specs/strategy/evolution/`. Never installs anything directly. |
| **Evolution Proposal**   | A Markdown report authored by the Scout, landing in `specs/strategy/evolution/`. Requires Chief approval before any tool can be integrated.                                               |
| **Genetic Guardian**     | The Chief Agent's role when reviewing Evolution Proposals: approve only tools that align with this Master Plan's Business Goals; reject everything else.                                  |
| **Agent Lineage**        | The generational succession model (Gen-1 → Gen-2 → Gen-3) where a parent agent distils its wisdom and spawns a cleaner child agent when context drift or a paradigm shift demands it.     |

---

## Evolution Protocol (Agent Lineage Architecture)

> **Purpose:** Prevent context degradation and paradigm lock-in by allowing the AI Swarm to evolve generationally while remaining anchored to this Spine.

### The Three Laws of Evolution

1. **The Scout Proposes, the Chief Disposes.**
   The Scout Agent (`python factory.py scout`) may only _propose_ tools. It never installs, imports, or modifies code. The Chief reviews proposals against this Master Plan and must explicitly schedule a Task Force to act on them.

2. **Alignment over Novelty.**
   A new tool is only worth integrating if it directly closes a gap listed in the **Current Gaps** section or measurably advances a **Business Goal**. No tool may be added for its own sake.

3. **Lineage Spawning Threshold.**
   A Gen-N → Gen-(N+1) spawn is warranted when **two or more** of the following signals are true:
   - The Scout's `lineage_note` field flags a paradigm shift (e.g., React → post-React framework).
   - `docs/LEARNED_GUIDELINES.md` has accumulated > 50 entries (context saturation).
   - A major dependency (e.g., Python version, core LLM API) requires a breaking migration.

### Spawn Procedure

When the Chief determines a spawn is warranted, it schedules a `task_force` with the goal:

> "Distil current learnings into a clean DNA package: new system prompts, compacted Lore, and updated Architecture contracts. Prepare handoff for Gen-N+1."

The Task Force:

1. **Steerer** — audits all `specs/` and resolves conflicts; produces a clean `master_plan_v<N+1>.md`.
2. **Scribe** — regenerates `docs/ARCHITECTURE.md` and compacts `LEARNED_GUIDELINES.md` to ≤ 30 essential rules.
3. **Repo Agent** — commits the "Generation N Archive" on a long-lived branch before the child wakes.
