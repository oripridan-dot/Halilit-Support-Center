...
def get_catalog_stats() -> str:
    """Returns live catalog statistics: brand count, product counts per brand, stale items."""
    data_dir = REPO_ROOT / "frontend" / "public" / "data"
    exclude = {"index", "search_index", "search_index_min", "galaxy_db", "sample", "inventory"}
    if not data_dir.exists():
        return "(catalog data directory not found)"
    brand_files = [f for f in data_dir.glob("*.json") if f.stem not in exclude]
    stats: dict = {"brand_count": len(brand_files), "brands": {}}
    total_products = 0
    for bf in sorted(brand_files):
        try:
            ...
            stats["brands"][bf.stem] = count
        except Exception as e:
            stats["brands"][bf.stem] = f"(error: {e})"
...
def get_ingestion_status() -> str:
    """Returns the last ingestion run status from the simulation ledger or data files."""
    ledger_candidates = [
        REPO_ROOT / "backend" / "data" / "simulation_ledger.json",
        REPO_ROOT / "backend" / "data" / "ingestion_ledger.json",
        REPO_ROOT / "backend" / "data" / "ingestion" / "last_run.json",
    ]
    for p in ledger_candidates:
        if p.exists():
            try:
                data = json.loads(p.read_text(encoding="utf-8"))
                return json.dumps({"source": str(p.relative_to(REPO_ROOT)), "data": data}, indent=2)
            except Exception as e:
                return f"(error reading {p.name}: {e})"
    # Fallback: check data dir mtime
    data_dir = REPO_ROOT / "backend" / "data"
    if data_dir.exists():
        import time
        mtime = max((f.stat().st_mtime for f in data_dir.rglob("*") if f.is_file()), default=0)
        return json.dumps({
            "status": "inferred",
            "last_data_file_modified": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(mtime)),
            "note": "No ingestion ledger found. Timestamp inferred from data dir mtime.",
        }, indent=2)
    return "(no ingestion status available)"
