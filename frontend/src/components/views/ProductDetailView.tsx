...
/**
 * Formats a number as a Shekel currency string.
 * Returns "—" if the number is null, undefined, or 0.
 */
function formatShekel(amount: number | null | undefined): string {
  if (amount == null || amount === 0) return "—";
  return `₪${amount.toLocaleString()}`;
}

/**
 * Checks if a price is "Call for Price" (CFP).
 * @param price The price to check.
 * @returns True if the price is CFP (null, undefined, or 0), false otherwise.
 */
function isCallForPrice(price: number | null | undefined): boolean {
...
/**
 * A skeleton header component for when product data is loading.
 */
...
interface RelatedProductCardProps {
  /** The related product item to display. */
  item: RelatedProduct;
  /** An optional badge to display on the card. */
  badge?: string;
  /** The color of the badge. */
  badgeColor?: string;
  /** A callback function to execute when the card is selected. */
  onSelect: () => void;
}

/**
 * A card component for displaying a related product.
 */
...
interface MiniSectionProps {
  ...
  title: string;
  /** The count to display in the mini section's badge. */
  count: number;
  /** The content of the mini section. */
  children: React.ReactNode;
}

/**
 * A section component with a title and a badge displaying a count.
 */
...
/**
 * A badge component indicating the source of data (Commercial, Official, Contextual).
 */
...
interface TabButtonProps {
  /** Whether the tab button is currently active. */
  active: boolean;
  /** A callback function to execute when the tab button is clicked. */
  onClick: () => void;
...
/**
 * A button component for a tab in a tabbed interface.
 */
 ...
 * including its specifications, related products, and history.
 */
const ProductDetailView: React.FC<ProductDetailViewProps> = () => {
  ...
  const { cancelStream: cancelJITStream, ...jitState } = useJITIntelligence(
    activeProductId ?? ""
  );
  ...
  const jitSpecs = jitState.delivery?.specs;
  ...
  const isStreaming =
    jitState.phase === "snap" ||
    jitState.phase === "promise" ||
    jitState.phase === "deliver";
  ...
  const isRenderedPartial =
    srcCommercial &&
    (!srcOfficial || !srcContextual) &&
    (jitState.phase === "error" ||
      jitState.phase === "complete" ||
      jitState.phase === "idle");
                ...
                {isStreaming && (
                  <div className="flex items-center gap-1.5 mt-1 shrink-0">
                    ...
                    <span className="text-[10px] text-blue-400">
                      {jitState.promise?.message || "Loading intelligence…"}
                    </span>
                  </div>
                )}
              </div>
              ...
              {isCallForPrice(displayPrice) ? (
                ...
                <div>
                  <div className="text-[28px] font-bold text-zinc-100 leading-none">
                    {formatShekel(displayPrice)}
                  </div>
                  {displayEilat && (
                    <div className="text-sm text-zinc-500 mt-0.5">
                      Eilat: {formatShekel(displayEilat)}
                    </div>
                  )}
                </div>
              )}
