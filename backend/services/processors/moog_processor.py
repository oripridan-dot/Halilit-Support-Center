...
class MoogProcessor:
    def normalize(self, raw_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        features = [f.strip() for f in raw_payload.get(
            'features', []) if f and f.strip()]
        ...
        # Accept both 'manuals' and 'manual_urls' keys from scrapers
        manual_urls = raw_payload.get(
            'manual_urls') or raw_payload.get('manuals', [])
        ...
        blueprint = {
            ...
            # Inference fallback
            "main_category": raw_payload.get('hierarchy', {}).get('category', 'Synths'),
            ...
            "manual_urls": manual_urls,  # Accept both PDFs and HTML documentation
