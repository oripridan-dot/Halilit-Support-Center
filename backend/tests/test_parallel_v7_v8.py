...
class V76TestRunner:
    ...
    def test_harvest(self, brand: str) -> ExecutionMetrics:
        ...
        metrics = ExecutionMetrics(
            version='v7.6', operation='harvest', brand=brand)
        ...
        try:
            ...
            logger.info(
                f"[v7.6] ✅ Harvested {len(products)} products in {metrics.duration_seconds:.2f}s")
    ...
    def test_enrich(self, products: List[Dict], brand: str) -> ExecutionMetrics:
        ...
        metrics = ExecutionMetrics(
            version='v7.6', operation='enrich', brand=brand)
        ...
        try:
            ...
            logger.info(
                f"[v7.6] Starting enrichment for {len(products)} products")
            enriched = []
            ...
            logger.info(
                f"[v7.6] ✅ Enriched {len(enriched)} products in {metrics.duration_seconds:.2f}s")
    ...
    def test_validate(self, products: List[Dict], brand: str) -> ExecutionMetrics:
        ...
        metrics = ExecutionMetrics(
            version='v7.6', operation='validate', brand=brand)
        ...
        try:
            ...
            logger.info(
                f"[v7.6] Starting validation for {len(products)} products")
            validated = []
            ...
            logger.info(
                f"[v7.6] ✅ Validated {len(validated)} products in {metrics.duration_seconds:.2f}s")
    ...
    def test_full_pipeline(self, brand: str) -> Tuple[List[ExecutionMetrics], bool]:
        ...
        # Enrich
        enrich_metrics, enriched_products = self.test_enrich(
            harvest_products, brand)
        metrics_list.append(enrich_metrics)

        # Validate
        validate_metrics, validated_products = self.test_validate(
            enriched_products, brand)
...
class V80TestRunner:
    ...
    async def test_harvest(self, brand: str) -> ExecutionMetrics:
        ...
        if not self.ready:
            metrics = ExecutionMetrics(
                version='v8.0', operation='harvest', brand=brand)
            metrics.complete(
                success=False, error='Celery broker not available')
            return metrics
        ...
        try:
            ...
            logger.info(
                f"[v8.0] Queuing harvest for {brand} (task_id={task_id})")
            ...
            try:
                ...
                logger.info(
                    f"[v8.0] ✅ Harvested {len(products)} products in {metrics.duration_seconds:.2f}s")
                return metrics
...
class TestCoordinator:
    ...
    def run_parallel_harvest(self, brands: List[str]) -> None:
        ...
        for brand in brands:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                ...
                v76_future = executor.submit(
                    self.v76_runner.test_harvest, brand)

                # V8.0 runs async (simulate with thread)
                v80_future = executor.submit(
                    asyncio.run, self.v80_runner.test_harvest(brand))
    ...
    def _compare_harvest_results(self, v76_metrics: ExecutionMetrics,
        ...
        logger.info(
            f"  v7.6: {v76_metrics.duration_seconds:.2f}s ({v76_metrics.product_count} products)")
        logger.info(
            f"  v8.0: {v80_metrics.duration_seconds:.2f}s ({v80_metrics.product_count} products)")
        logger.info(
            f"  Performance delta: {perf_delta:+.1f}% {'(v8.0 faster ✅)' if perf_delta > 0 else '(v7.6 faster)'}")
        logger.info(
            f"  Accuracy match: {'✅ YES' if accuracy_match else '❌ NO'}")
    ...
    def print_summary(self):
        ...
        print(
            f"Accuracy Match: {passed}/{total} ({'✅ PASS' if passed == total else '❌ FAIL'})")
        print(
            f"Avg Performance Delta: {sum(r.performance_delta for r in self.results) / len(self.results):.1f}%\n")
        ...
        for result in self.results:
            ...
            print(
                f"    v7.6: {result.v7_metrics.duration_seconds:7.2f}s ({result.v7_metrics.product_count:3} items)")
            print(
                f"    v8.0: {result.v8_metrics.duration_seconds:7.2f}s ({result.v8_metrics.product_count:3} items)")
            print(
                f"    Perf: {result.performance_delta:+6.1f}% | Accuracy: {'✅' if result.accuracy_match else '❌'}\n")
        ...
        results_data = {
            ...
            'summary': {
                ...
                'avg_performance_delta': round(
                    sum(r.performance_delta for r in self.results) /
                    len(self.results),
                    2
                )
            }
