...
class MassIngestProtocol:
    ...
    def run_brand_pipeline(self, brand_name, scraper_class, processor_class):
        if self.mode != "official_only":
            print(
                f"⏩ [SKIP] Skipping Official Brand Pipeline for {brand_name} (Mode: {self.mode})")
            return
        ...
        if hasattr(scraper, 'scrape_and_return_raw'):
            ...
            raw_items = asyncio.run(scraper.scrape_and_return_raw())
        else:
            print(
                f"⚠️ Scraper for {brand_name} does not have 'scrape_and_return_raw'.")
            return
        ...
        for raw_wrapper in saved_raw_files:
            ...
            try:
                ...
                audit = self.auditor.audit_product(
                    raw_wrapper['raw_payload'], blueprint)
                brand_audit_results.append(audit)

                if audit['missing_critical']:
                    print(
                        f"⚠️  [GAP] {audit['model']} missing: {audit['missing_critical']}")
                if audit['extra_unmapped_data']:
                    print(
                        f"💎 [DISCOVERY] {audit['model']} has unused data: {audit['extra_unmapped_data']}")

            except Exception as e:
                print(
                    f"❌ Error processing {raw_wrapper['metadata']['model']}: {e}")

        # Save Audit Report
        if brand_audit_results:
            report_path = self.auditor.save_brand_report(
                brand_name, brand_audit_results)
        ...
        if processed_blueprints:
            ...
            with open(bp_path, 'w', encoding='utf-8') as f:
                json.dump(processed_blueprints, f,
                          indent=2, ensure_ascii=False)
            print(
                f"💾 [SAVED] {len(processed_blueprints)} blueprints to {bp_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", choices=["official_only", "commercial_sync"], default="official_only", help="Ingestion Mode")
