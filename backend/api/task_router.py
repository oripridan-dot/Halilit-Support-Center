"""
FastAPI Task Router for Halilit Support Center v8.0
...
# Create router with v8.0 prefix
...
class TaskStatusResponse:
    ...
    def __init__(self, task_id: str, result: AsyncResult):
        ...
        if result.state == 'PROGRESS':
            self.progress = result.info.get('status') if isinstance(result.info, dict) else None
        ...
        if result.ready():
            if result.successful():
                ...
                self.result = None
                self.error = str(result.info) if result.info else 'Unknown error'
        else:
            self.result = None
            self.error = None
...
async def queue_harvest(brand: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"📋 Queueing harvest for brand: {brand} (task_id={task_id})")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to queue harvest: {str(e)}")
...
async def get_harvest_result(task_id: str, poll_interval: int = Query(1, ge=1, le=60)) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug(f"📊 Fetching result for task {task_id}: state={result.state}")
...
async def queue_batch_sync(brand: str, product_ids: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"🔗 Queueing batch sync for brand: {brand} (sync_id={sync_id})")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to queue batch sync: {str(e)}")
