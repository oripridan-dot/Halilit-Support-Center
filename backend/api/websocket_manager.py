        ...
        try:
            ...
            if result.state == 'PROGRESS':
                if isinstance(result.info, dict):
                    status['progress'] = result.info.get('status', 'processing')
        ...
        while True:
            ...
            async with self.lock:
                if task_id not in self.task_subscriptions or not self.task_subscriptions[task_id]:
                    logger.info(f"🛑 No subscribers for task {task_id}, stopping poll")
                    break
            ...
            try:
                ...
                if current_state != last_state:
                    ...
                    if result.ready():
                        logger.info(f"✅ Task {task_id} completed (state={current_state})")
                        break
                ...
                logger.warning(f"⚠️ Poll error for task {task_id}: {e} (attempt {consecutive_errors})")
                
                if consecutive_errors >= 5:
                    logger.error(f"❌ Giving up on task {task_id} after 5 consecutive errors")
                    break
