...
def sync_brand_to_frontend(brand: str) -> tuple[bool, List[Dict[str, Any]]]:
    ...
    ⭐ STRICT VALIDATION: Only writes products that have:
       - Valid price_il (> 500 NIS for real products)
       - At least 1 official image
    ...
    try:
        ...
        # ⭐ STRICT VALIDATION PASS: Filter to only write high-quality products
        valid_products = []
        invalid_count = 0
        ...
        if invalid_count > 0:
            logger.warning(
                f"  ⚠️  Filtered out {invalid_count} products below quality threshold")

        # Write ONLY valid products to frontend
